...
from LOFTER.config import LOFTERSettings


PER_PAGE = LOFTERSettings['pagination'].get('per_page', 10)
ARCHIVE_PER_PAGE = LOFTERSettings['pagination'].get('archive_per_page', 10)


def get_base_data():
    pages = models.Post.objects.filter(post_type='page', is_draft=False)
    blog_meta = LOFTERSettings['blog_meta']
...
def post_detail(slug, post_type='post', fix=False, is_preview=False):
    ...
    if request.form.get('loft-comment') and form.validate_on_submit():
        lofter_create_comment(form, post)
    ...
    data['allow_donate'] = LOFTERSettings['donation']['allow_donate']
    data['donation_msg'] = LOFTERSettings['donation']['donation_msg']

    data['display_wechat'] = LOFTERSettings['wechat']['display_wechat']
    data['wechat_msg'] = LOFTERSettings['wechat']['wechat_msg']

    data['display_copyright'] = LOFTERSettings['copyright']['display_copyright']
    data['copyright_msg'] = LOFTERSettings['copyright']['copyright_msg']

    data['allow_comment'] = LOFTERSettings['blog_comment']['allow_comment']
    if data['allow_comment']:
        comment_type = LOFTERSettings['blog_comment']['comment_type']
        comment_shortname = LOFTERSettings['blog_comment']['comment_opt'][comment_type]
    ...
    data['allow_share_article'] = LOFTERSettings['allow_share_article']
...
def get_comment_func(comment_type):
    ...
    #     return None
    comment_func = {
        'lofter': lofter_comment,
        'duoshuo': duoshuo_comment,
    }

    return comment_func.get(comment_type)

def lofter_comment(post_id, post_title, post_url, comment_shortname, form=None, *args, **kwargs):
...
def lofter_create_comment(form, post):
...
def recent_feed():
    feed_title = LOFTERSettings['blog_meta']['name']
    ...
    only_abstract_in_feed = LOFTERSettings['only_abstract_in_feed']
